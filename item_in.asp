		...
		<td width="917" height="585">
			<table width="917" height="585" border="0" cellpadding="0" cellspacing="0">
				...
				<tr>
					...
					'''''模型表名称
					Dim ModelTable:ModelTable ="user_item"
					...
					set rs = UICon.Query("select * from user_item where ID="&ItemID&"")
					UIcon.execute("update user_item set Hots=Hots +1 where id="&ItemID&"")
					...
					<tr>
					<td height="30" align="center" valign="middle" class="lan" ><h4><%=rs("title")%></h4></td>
					</tr>
					<tr>
					<td align="left" valign="middle"></td>
					</tr>
					...
					%>
					<div style="margin:10px auto; text-align:center"><img src="<%=rs("Field_picture")%>" width="300"></div>
					<% Else %>
					&nbsp;
					<% End If %>
					<%
					...
					上一个产品：
					<%
					set rsu = Uicon.query("select top 1 id,categoryid,title from user_item where categoryid="&categoryid&" and id<"&itemid&" order by id desc")
					...
					<td height="26">下一个产品：
					<%
					set rsn = Uicon.query("select top 1 id,categoryid,title from user_item where categoryid="&categoryid&" and id>"&itemid&" order by id asc")
