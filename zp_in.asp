...
<table width="1366" height="1127" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		<td width="917" height="585">
			<table width="917" height="585" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td>
						<img src="images/main_in_01zp.jpg" width="743" height="57" alt=""></td>
	...
	'''''模型表名称
	Dim ModelTable:ModelTable ="user_zp"
...
%>
  <%
			...
			set rs = UICon.Query("select * from user_zp where ID="&ItemID&"")
			UIcon.execute("update user_zp set Hots=Hots +1 where id="&ItemID&"")
  ...
  <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td height="30" align="center" valign="middle" class="lan"><h2><%=rs("title")%></h2></td>
                    </tr>
        <tr>
		  ...
		  <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td height="35" align="left" valign="middle" class="news_fabutime" style="font-weight: bold">发布时间：<%=rs("posttime")%></td>
                ...
                上一篇招聘信息：
                    <%
		set rsu = Uicon.query("select top 1 id,categoryid,title from user_zp where categoryid="&categoryid&" and id<"&itemid&" order by id desc")
		...
		if rsu.recordcount<>0 then 
			...
			response.Write "已经是第一篇招聘信息了"
		end if
		%></td>
                </tr>
    <tr>
      <td height="26">下一篇招聘信息：
        <%
		set rsn = Uicon.query("select top 1 id,categoryid,title from user_zp where categoryid="&categoryid&" and id>"&itemid&" order by id asc")
		if rsn.recordcount<>0 then 
			response.Write "<a href='?categoryid="&rsn("categoryid")&"&itemid="&rsn("id")&"'  >"&rsn("title")&"</a>"
		else
			response.Write "已经是最后一篇招聘信息了"
		end if
		%></td>
                </tr>
    <tr>
