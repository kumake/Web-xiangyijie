	...
	<tr>
		...
		<td width="917" height="585">
			<table width="917" height="585" border="0" cellpadding="0" cellspacing="0">
				...
				if total = 0 then 
				Dim Tarray:Tarray = UICon.QueryData("select count(*) as total from user_zp where "&strwhere&"")
				...
				'Dim sql:sql = UICon.QueryPageByNum("categoryID,id,title,posttime","user_zps", ""&strwhere&"", "ID", true,current_page,pagesize)
				'这种方式为根据IndexID排序，IndexID值越大越靠前
				Dim sql:sql = UICon.QueryPageByNotIn("*","user_zp", ""&strwhere&"", "ID", "indexID desc,ID",false,current_page,pagesize)
				'response.Write sql
				'response.End()
				...
				<span style="font-family:'宋体'">·</span><a href="zp_in.asp?categoryid=<%=rsn("categoryID")%>&amp;itemid=<%=rsn("id")%>" class="y_0"><%=rsn("title")%></a> </td>
				</tr>
				<tr>
