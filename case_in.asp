		...
		<td width="917" height="585">
			<table width="917" height="585" border="0" cellpadding="0" cellspacing="0">
				...
				<tr>
					...
					'''''模型表名称
					Dim ModelTable:ModelTable ="user_case"
					...
					UIcon.execute("update user_case set Hots=Hots +1 where id="&ItemID&"")
					...
					<tr>
					<td align="center" valign="middle" class="lan" ><h4><%=rs("title")%></h4></td>
					</tr>
					<tr>
					<td align="left" valign="middle"></td>
					</tr>
					...
					%>
					<div style="margin:10px auto; text-align:center"><img src="<%=rs("Field_picture")%>" width="350"></div>
					<% Else %>
					&nbsp;
					<% End If %>
					<%
					...
					上一个案例：
					<%
					set rsu = Uicon.query("select top 1 id,categoryid,title from user_case where categoryid="&categoryid&" and id<"&itemid&" order by id desc")
					...
					<td height="26">下一个案例：
					<%
					set rsn = Uicon.query("select top 1 id,categoryid,title from user_case where categoryid="&categoryid&" and id>"&itemid&" order by id asc")
